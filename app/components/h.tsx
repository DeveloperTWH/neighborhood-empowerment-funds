...
export default function Hero() {
  // Refs for left and right image divs
  const leftImagesRefs = useRef<(HTMLDivElement | null)[]>([]);
  const rightImagesRefs = useRef<(HTMLDivElement | null)[]>([]);

  // Ref for text content container
  const textRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    // Animate text content fade-in and slide from left
    if (textRef.current) {
      gsap.fromTo(
        textRef.current,
        { autoAlpha: 0, x: -50 },
    ...
    // Animate left column images from bottom with stagger
    if (leftImagesRefs.current.length) {
      gsap.fromTo(
        leftImagesRefs.current.filter(Boolean),
    ...
    // Animate right column images from bottom with stagger, delay for effect
    if (rightImagesRefs.current.length) {
      gsap.fromTo(
        rightImagesRefs.current.filter(Boolean),
        { autoAlpha: 0, y: 50, rotation: -5 },
        {
          autoAlpha: 1,
          y: 0,
          ...
          <div className="flex flex-col gap-4 md:gap-6">
            {imagesRightColumn.map((src, idx) => (
              <div
                key={idx}
                ref={(el) => {
                  rightImagesRefs.current[idx] = el;
                }}
                className="transition-transform duration-300 ease-in-out hover:scale-95 hover:-rotate-2"
              >
                <Image
